...
- 第一题直接a
- 第二题是 一个长度为5的list，为－1 能减多少回，不可以减成-1，用的排序，每次减最后四个数，不知道问题出在哪里，但是 应该用堆排序，每次减他，再入堆。 这个数据结构很适合处理，一个一个减的问题，就是挨个减，挨个什么的处理。

## VIVO

- 和平安差不多，就是介绍项目，好像这个公司做落地比较多，对我的研究不太感兴趣
- 问了模型参数如何变少，模型压缩，剪枝（待了解）
- 用你学的知识这几一个产品，或者一个功能。

## 上海人工智能实验室-商汤

- 数据不均衡怎么解决
- 10000类的分类怎么做-说的用树状结构-他给予肯定
- triplet loss 的变种，有很多 在reID里面，不知道 找一找
- ReID的难点问题都有什么，给一个不知道的人介绍一下reid
- 他好像对分割不感兴趣
- 总结REID的方法分为哪几种，无论什么REID 任务都可以用的那种方法，我说从全局特征，局部特征角度，mask-based 角度等等
- 最后考了个DP 从左上到右下找一条和最小路径
- BN实现一下，用 Numpy
- array 和 list的区别，array类型唯一，list 可以是多种类型
- 抛硬币， 正反他赢 反反我赢， 问概率， 1/4, 3/4
- 问我两个链表怎么判断相交
- 有时候好像有点答不对题，注意他问什么以后
- 他一直在注重是不是有自己搞过模型，优化过模型，还是只是应用阶段
- yolo 1，2，3 的区别， 训练策略的区别，挑选正负样本
- mobilenetv1 v2的优缺点
- 归一化和标准化什么区别
- 多看看面经，应该都是常规问题
- 估计凉了，但是还是体验不错，去看一下triplet loss的变种
- 对他有什么问题，上海什么什么实验室是个啥，他说就是商汤和合作，要求一样。
- 感受野计算 3*3 深度为3的，步长1，感受野多少，可以像 第一层是3，步长1，扩到第二层，会+2，再扩还会+2，如果stride为其他的也看看扩了多少出来。
- 深度可分离卷积，和普通卷积，参数都是多少，距离，3 到 256通道的操作。
- 实习项目问我数据量，达到什么精度之类的，太久远了，没怎么准备，再看看。
- SVM的核函数，了解多少，什么时候用什么核
